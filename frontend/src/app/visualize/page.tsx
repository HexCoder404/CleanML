...
function BoxPlotBar({ col, data }: { col: string; data: any }) {
  ...
  return (
    <div className="mb-6">
      ...
      <div className="relative w-full h-10 bg-gray-100 rounded-full">
        ...
        <div className="absolute top-0 bottom-0 w-0.5 bg-indigo-700" style={{ left: `${medp}%` }} />
      </div>
...
function CorrHeatmap({ cols, data }: { cols: string[]; data: any[] }) {
      ...
      <table className="border-collapse text-xs">
        <thead>
          <tr>
            <th className="p-1"></th>
            {cols.map((c) => (
              <th key={c} className="p-1 text-gray-500 font-medium" style={{ writingMode: "vertical-rl", maxWidth: 60 }}>{c}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {cols.map((r) => (
            <tr key={r}>
              <td className="pr-2 text-gray-500 font-medium whitespace-nowrap text-right" style={{ maxWidth: 100, overflow: "hidden", textOverflow: "ellipsis" }}>{r}</td>
              {cols.map((c) => {
                const v = lookup[`${r}|||${c}`] ?? null;
                return (
                  <td key={c} className="w-12 h-10 text-center font-bold transition-all"
                    style={{ background: corrColor(v), color: v !== null && Math.abs(v) > 0.5 ? "#fff" : "#374151" }}
                    title={`${r} vs ${c}: ${v}`}>
                    {v !== null ? v.toFixed(2) : "—"}
                  </td>
                );
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans">
      <nav className="bg-white border-b border-gray-100 px-8 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        ...
        <div className="hidden md:flex space-x-6 text-sm font-medium text-gray-500">
          <a href="/" className="hover:text-indigo-600 transition-colors">Clean</a>
          <a href="/visualize" className="hover:text-indigo-600 transition-colors font-semibold text-gray-700">Visualize Data</a>
      ...
      <main className="max-w-7xl mx-auto px-8 py-10 space-y-14">
        ...
        <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center space-y-6">
          <div className="flex items-center justify-center w-full max-w-xl">
            <label
              ...
              className={`flex flex-col items-center justify-center w-full h-44 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${
                isDragging ? "border-indigo-500 bg-indigo-100 ring-4 ring-indigo-50" : "border-indigo-300 bg-indigo-50 hover:bg-indigo-100"
              }`}
            >
              <div className="flex flex-col items-center justify-center pt-5 pb-6">
                <svg className={`w-10 h-10 mb-4 ${isDragging ? "text-indigo-600 animate-bounce" : "text-indigo-500"}`} fill="none" viewBox="0 0 20 16" xmlns="http://www.w3.org/2000/svg">
          ...
          {file && <div className="text-sm font-medium text-gray-700 bg-gray-100 px-4 py-2 rounded-lg">Selected: {file.name}</div>}

          <button
            onClick={handleUpload}
            disabled={!file || uploading}
            className="px-8 py-3 text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-full font-semibold transition-all shadow-md hover:shadow-lg min-w-[200px] flex justify-center"
          >
            {uploading ? (
          ...
          {uploadError && <div className="text-red-500 text-sm font-medium p-4 bg-red-50 rounded-lg w-full max-w-xl text-center">{uploadError}</div>}
        </section>
        ...
        {error && <div className="bg-red-50 border border-red-200 text-red-600 rounded-xl p-6 text-center">{error}</div>}
          ...
          <>
            ...
            <section>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                <StatCard label="Total Rows"       value={vizData.overview.row_count.toLocaleString()} color="bg-indigo-500" />
                <StatCard label="Total Columns"    value={vizData.overview.column_count}              color="bg-blue-500"   />
                <StatCard label="Numeric Cols"     value={vizData.overview.numeric_count}             color="bg-emerald-500"/>
                <StatCard label="Categorical Cols" value={vizData.overview.categorical_count}         color="bg-amber-500"  />
              </div>
              <div className="grid md:grid-cols-2 gap-6">
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                  <h3 className="text-sm font-bold text-gray-700 uppercase tracking-wide mb-4">Missing Values per Column</h3>
                  {vizData.overview.missing_values.length === 0 ? (
                    ...
                    <ResponsiveContainer width="100%" height={220}>
                      <BarChart data={vizData.overview.missing_values} layout="vertical" margin={{ left: 16, right: 24, top: 4, bottom: 4 }}>
                        ...
                        <YAxis type="category" dataKey="column" width={100} tick={{ fontSize: 11 }} />
                ...
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                  <h3 className="text-sm font-bold text-gray-700 uppercase tracking-wide mb-4">Data Type Distribution</h3>
                  <ResponsiveContainer width="100%" height={220}>
            ...
            {Object.keys(vizData.histograms).length > 0 && (
              <section>
                <SectionHeader title="Numeric Features" icon="📊" />
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-sm font-bold text-gray-700 uppercase tracking-wide">Histogram</h3>
                      <select value={activeHistCol} onChange={(e) => setActiveHistCol(e.target.value)} className="text-sm border border-gray-200 rounded-md px-2 py-1 text-gray-700">
                        {Object.keys(vizData.histograms).map((c) => <option key={c} value={c}>{c}</option>)}
                      </select>
                    </div>
                    {activeHistCol && (
                      <ResponsiveContainer width="100%" height={220}>
                        <BarChart data={vizData.histograms[activeHistCol]} margin={{ top: 4, right: 8, bottom: 40, left: 4 }}>
                          <CartesianGrid strokeDasharray="3 3" vertical={false} />
                          <XAxis dataKey="bin" tick={{ fontSize: 9 }} angle={-35} textAnchor="end" interval={2} />
                          <YAxis tick={{ fontSize: 11 }} />
                          <Tooltip />
                  ...
                  <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-sm font-bold text-gray-700 uppercase tracking-wide">Box Plot — Outlier Detection</h3>
                      <select value={activeBoxCol} onChange={(e) => setActiveBoxCol(e.target.value)} className="text-sm border border-gray-200 rounded-md px-2 py-1 text-gray-700">
                        {Object.keys(vizData.boxplots).map((c) => <option key={c} value={c}>{c}</option>)}
                      </select>
                    </div>
                    {activeBoxCol && <BoxPlotBar col={activeBoxCol} data={vizData.boxplots[activeBoxCol]} />}
                    <div className="mt-4 grid grid-cols-3 gap-2 text-xs text-center">
                      {activeBoxCol && ([
                        ...
                        <div key={l} className="bg-gray-50 rounded-lg p-2 border border-gray-100">
                          ...
                          <p className="text-gray-800 font-bold">{typeof v === "number" ? Number(v).toFixed(2) : v}</p>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {Object.keys(vizData.categorical).length > 0 && (
              <section>
                <SectionHeader title="Categorical Features" icon="🏷️" />
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-sm font-bold text-gray-700 uppercase tracking-wide">Category Frequency — Top 10</h3>
                    <select value={activeCatCol} onChange={(e) => setActiveCatCol(e.target.value)} className="text-sm border border-gray-200 rounded-md px-2 py-1 text-gray-700">
                      {Object.keys(vizData.categorical).map((c) => <option key={c} value={c}>{c}</option>)}
                    </select>
                  </div>
                  {activeCatCol && (
            ...
            {vizData.correlation.columns.length >= 2 && (
              <section>
                <SectionHeader title="Correlation Heatmap" icon="🔗" />
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                  ...
                  <CorrHeatmap cols={vizData.correlation.columns} data={vizData.correlation.data} />
                </div>
              </section>
            )}
          </>
