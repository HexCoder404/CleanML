...
export default function AuthModal({ isOpen, onClose, onSuccess, initialMode = "signup" }: AuthModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (mode === "signup" && email.toLowerCase() === "adityakakad10@gmail.com") {
      setError("This email is registered. Please log in.");
      return;
    }
    ...
    if (mode === "login" && email.toLowerCase() === "adityakakad10@gmail.com" && password !== "Clean4Luck") {
    ...
    setTimeout(() => {
      try {
        const isAdmin = email.toLowerCase() === "adityakakad10@gmail.com";
        const user = {
          email,
          name: isAdmin ? "Aditya" : (name || email.split("@")[0]),
          isLoggedIn: true,
  ...
  const handleSocialLogin = (provider: string) => {
    ...
    setTimeout(() => {
      const user = {
        email: `${provider.toLowerCase()}user@cleanml.ai`,
        name: `${provider} User`,
        isLoggedIn: true,
        role: "user",
      };
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="p-8">
          ...
          <div className="text-center mb-6">
            <div className="inline-flex items-center space-x-2 text-violet-600 mb-2">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" />
              </svg>
              <span className="text-xl font-extrabold tracking-tight">CleanML</span>
            </div>
          ...
          {/* Error Message */}
          {error && (
            <div className="mb-4 p-3 text-xs font-semibold text-rose-600 bg-rose-50 rounded-lg border border-rose-100 flex items-center gap-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === "signup" && (
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 text-sm transition-all"
                />
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 bg-violet-600 hover:bg-violet-700 text-white font-bold text-sm rounded-xl transition-all shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
            >
              {loading ? (
          ...
          <div className="relative my-6 text-center">
            ...
            <span className="absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 bg-white px-3 text-xs text-gray-400 font-medium uppercase tracking-wide">Or continue with</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              disabled={loading}
              className="flex items-center justify-center gap-2 py-2 border border-gray-200 rounded-xl hover:bg-gray-50 text-xs font-semibold text-gray-600 transition-all disabled:opacity-50"
            >
              {/* Google Icon SVG */}
